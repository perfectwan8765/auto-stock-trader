"""단계 3 진단 — 사전등록 §5 "검증"이 요구하는 층화·교차확인.

판정(run_event_study.py)과 분리한 이유: 여기 나오는 것은 **전부 진단**이고 판정에 쓰지 않는다.
고를 수 없으므로 §4.1의 세는 규칙에 따라 시도 예산 N에도 계상하지 않는다.

  (1) 모멘텀(price deviation) 버킷별 BHAR — **파이프라인 신뢰성 검사**.
      원논문 Table 4는 ≤0% → CAR 2.3% … >10% → 6.3% 의 단조 증가다.
      우리 표본에서 같은 방향의 단조성이 안 나오면 파이프라인을 의심해야 한다.
  (2) NANO(<$50M) 층화 — 결과가 나노캡에 끌려갔는지
  (3) 은행 제외 부표본 — 판정 표본의 24%가 은행이고 2023 지역은행 위기와 겹친다
  (4) raw vs winsor 1% 대조 — D17의 자유도 크기가 §3.13(3) 표와 같은지
  (5) calendar-time portfolio — D16의 교차확인. 결론이 뒤집히는지만 본다
...
def main() -> None:
    ...
    print("\n=== (1) 모멘텀(price deviation) 버킷 — 파이프라인 신뢰성 검사 ===")
    print("  ⚠️ normalized 가격은 첫 종가 기준으로 정규화돼 있어 원가격(거래단가)과 수준 비교가"
          " 불가능하다. 대신 **공시 전 20거래일 수익률**을 모멘텀 대리로 쓴다 —"
          " 원논문의 price deviation과 같은 방향의 지표다.")
    mom = []
