...
고를 수 없으므로 §4.1의 세는 규칙에 따라 시도 예산 N에도 계상하지 않는다.
  ...
  (1) 공시 전 모멘텀 버킷별 BHAR — **파이프라인 신뢰성 검사**. 원논문 Table 4의
      price deviation은 ≤0% → CAR 2.3% … >10% → 6.3% 로 단조 증가한다. 정규화 가격은
      거래단가와 수준 비교가 안 되므로 공시 전 20거래일 수익률을 대리로 쓴다.
      같은 방향이 안 나오면 파이프라인을 의심해야 한다.
