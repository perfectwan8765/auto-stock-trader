...
```
스크립트는 항상 프로젝트 루트에서 `uv run python scripts/toss_probe/XX.py` 로 실행.
...
```bash
# 1) 계획만 확인(안전)
uv run python scripts/toss_probe/04_place_test_order.py --symbol AAPL --amount 1
# 2) 정규장 시간에 실제 발주
uv run python scripts/toss_probe/04_place_test_order.py --symbol AAPL --amount 1 --confirm
```
에러코드 판독은 스크립트 출력 안내 및 `qlib-toss.md` §1 참고.

## 결과 기록
Phase 0은 2026-07-20 실측 완료. 확정값은 `qlib-toss.md` §Phase 0 실측 상수에 있다.
재측정 시 계정 고유값(계좌번호·잔액·보유)은 커밋하지 말고, 계정 무관한 API 계약·비용만 그 표에 갱신할 것.
