"""분기별 이벤트 커버리지 전수 측정 + Toss 폐지기록 교차검증 — 계획서 §5 단계 1(b)(c)(f).

§3.10(1)은 분기당 220종목 표본이었다. kill(1)("최근 구간조차 커버리지 80% 미달 → 종료")은
전수로 판정해야 하므로 주 표본 전 구간을 분기별로 다시 센다.

커버리지 = 이벤트 시점 가격 패널에 그 티커의 관측이 있는가. 티커 기준과 이벤트 가중 기준을
모두 낸다 — 둘이 다르면 결측이 이벤트 수와 상관되어 있다는 뜻이고 그 자체가 정보다(§11).
...
def _crosscheck_toss(ev: pd.DataFrame, covered: set[str]) -> None:
    """단계 1(f) — 가격 결측 티커가 Toss 쪽 폐지 기록과 겹치는지 본다.

    Toss `status`/`delistDate`는 yfinance와 독립 소스다. 결측이 실제 퇴출과 무관하다는
    §3.14(1)의 결론에 대한 교차검증이다.
    """
