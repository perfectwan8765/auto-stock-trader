...
- Alpha158 + LightGBM, config YAML 작성
- **train/valid/test 시간순 분리, test 최근 30% 격리**
- walk-forward 1회 이상
- **[개선2] 백테스트=실전 조건 미러링:** 주간 리밸런싱·K·롱온리·소수점(fractional) 포지션·**토스 실제 수수료(`GET /commissions`)**·환전비용을 백테스트에 반영. 임의 슬리피지(0.1%)는 하한 가정으로만
- **[개선7] no-trade band:** 목표비중 대비 편차 X%(예: 20%) 이상일 때만 거래하는 밴드로 회전율·비용 억제. 밴드값은 백테스트로 튜닝
- **검증 지표:** IC/Rank IC, IR, MDD, 회전율. **Sharpe 4+면 과최적 의심 → 재검토**
- **에지케이스:** lookahead bias·비현실적 지표·회전율 과다·백테스트-실전 조건 괴리
