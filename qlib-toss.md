...
- walk-forward 1회 이상
- **[개선2] 백테스트=실전 조건 미러링:** 주간 리밸런싱·K·롱온리·소수점(fractional) 포지션·**토스 실제 수수료(`GET /commissions`)**·환전비용을 백테스트에 반영. 임의 슬리피지(0.1%)는 하한 가정으로만. ⚠️ **실수수료는 Phase 0(키 승인 대기)에 의존** → 당장은 토스 공개 수수료표+보수적 슬리피지 placeholder, Phase 0 풀리면 교체
