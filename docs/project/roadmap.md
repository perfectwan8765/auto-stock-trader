...
| ★ 환전 **시간창** | **평일 09:00~15:30 KST에만 우대가.** 그 밖·주말·공휴일은 **약 10배** | 아래 주 |
...
> ★ **선환전은 반드시 평일 09:00~15:30 KST(서울외환시장 영업일)에 한다.**
> 토스 FAQ 3549 기준 기본 스프레드는 MAR 대비 1%이고, 평일 장중에만 95% 할인(실효 0.05%),
> 그 밖·주말·공휴일은 50% 할인(실효 0.5%)이다. **위 실측 0.03%는 평일 우대가를 잰 것**이며
> (`편도 0.5% × 5% = 0.025% ≈ 0.03%`로 정합), 시간대를 벗어나면 그 값이 성립하지 않는다.
>
> ⚠️ **Phase 6 스모크에서 실제로 걸리는 함정이다.** 미국 정규장은 한국시간 22:30~05:00라
...
Phase 6 스모크의 관찰 항목이다(아래 "Phase 6 관찰 체크리스트").
...
- `kernels=1`이면 즉시 완료된다. ★ **느려지지 않는다 — 오히려 빠르다.** 533종목 전수 스캔
  (60일 창) 실측으로 `기본값 6.9s` vs `kernels=1 0.3s`로 **23배 차이**다. 이 규모에서는
  워커 스폰 비용이 작업량을 압도하므로 멀티프로세싱이 순손해다
...
- **[개선4] 안전장치:** `--dry-run`(주문 대신 발주계획 로그만) · 파일/env 기반 **kill switch**(존재 시 즉시 중단) · **서킷브레이커**(일일 주문건수·손실 상한 초과 시 정지)
  - ★ **safe state는 `frozen`(동결)이지 `flat`(청산)이 아니다.** long-only 현물에서 자동 청산은
    왕복 20bp를 확정 실현시키고 저점매도를 고착시킨다 — 안전장치가 손실을 만드는 경로다.
    **자동 손실 청산 로직은 넣지 않는다.** 정지는 "더 이상 주문하지 않음"까지이며,
    포지션 정리는 사람이 판단한다
...
- **[개선11] invalid-token 재시도 ✅(2026-07-20 구현, mock 검증):** `TossClient.request()`가 401 응답 시 `get_token(force_refresh=True)` 후 **1회만** 재시도(`_retry` 플래그로 상한). 401은 미처리 거부라 POST /orders 재시도도 안전(clientOrderId 멱등키 이중안전망). mock 테스트 3(재시도·상한·비401 무재시도). **실 401 동작 확인만 Phase 0(키 승인 후).**
  ...
  재발급 토큰을 직접 넘기고, 만료를 몰라 캐시할 수 없으면 옛 항목을 지운다(남겨두면 다음
  요청마다 401을 한 번씩 더 맞는다). 근거는 커밋 `3f231dc`·`1558883` 본문.
...
- ✅ **E10 서킷브레이커 파일영속** — 인메모리만 쓰면 상한에 걸려 멈춘 뒤 재기동하는 것만으로
  ...
  근거는 커밋 `db224bd`(일일 경계)·`7e59422`(워터마크)·`3af7520`(매수 한정)·`97faa89`(기본값) 본문
...
### Phase 6 관찰 체크리스트 (2026-08-17 확정)

실계좌 dry-run으로는 **확인할 수 없어** 첫 실발주로 넘긴 것들이다. dry-run은 관리셋이 비면
`get_sellable`을 아예 호출하지 않고, `observe_daily_loss`는 실발주 전용 경로에 있다.
⚠️ 결과를 적을 때 **계좌 고유값(잔액·보유 종목·수량)은 쓰지 않는다** — 이 문서는 추적 대상이다.

| 관찰 항목 | 왜 필요한가 | 결과에 따른 조치 |
|---|---|---|
| `sellable-quantity` 응답에 `sellableQuantity`가 있는가 | 어댑터가 "모름"을 `None`으로 돌려주는 조치의 전제. 트리거 미관측 | 없으면 러너의 미조회 가드가 실제로 발동한다 — 원인 조사는 별건 |
| 보유 수량에 1e-4 미만(dust)이 있는가 | exit 최소수량 하한을 근거 없이 미도입 상태로 뒀다 | 있으면 하한 도입 재평가 |
| `dailyProfitLoss.amount`가 USD 자릿수인가 | `--max-loss 70`이 이 통화 가정 위에 서 있다 | 아니면 즉시 재산정. 워터마크라 해제는 `circuit_breaker.json` 삭제뿐 |
| `execution` 숫자 필드가 실제로 채워지는가 | 필드명이 틀리면 `Fill`이 전부 `None`인데 미체결과 구별되지 않는다 | 비어 있으면 실응답과 대조해 필드명 교체 |
| `settlementDate` 필드명·포맷(날짜만/ISO/정수) | 유일하게 검증 없이 원시 통과하는 필드 | 포맷 확정 후 `_num`/`_opt_num`에 준하는 정규화 도입 |
| 실 결제일이 T+1인가 T+2인가 | 세법상 양도시기·환율기준일의 근거 | 확정 후 `interface.py`·`test_broker.py`의 중복 서술을 한쪽으로 합침 |
| 동일 `clientOrderId` 재발주 응답이 신규/기존을 구분하는가 | 재실행이 주문 카운터를 이중계상하는지의 전제 | 구분되면 `record_order` 조건부화 |
| **건당 최소수수료** | Phase 0이 최소 *주문금액*만 쟀다. $1이면 $35 포지션에 편도 2.9%라 **어떤 전략도 성립하지 않는다** | no-trade 밴드·최소주문금액·K 전부 재계산 |
| 1회 리밸의 실제 주문 건수 / 손실 상한 트립 빈도 | `--max-orders 60`·`--max-loss 70`을 근거 없이 유지 중 | 상한 재산정 |

### 실행 계층 잔여 항목 (2026-08-17 리뷰 후)

의도적으로 닫지 않은 것들이다. 각각 근거가 있다 — 상세는 커밋 `847102b` 본문.

- **발주 중 네트워크 예외로 주문 성사 불명** — 근본책은 발주 전 intent WAL(`docs/research/dashboard-features.md` "POST 전 flush")
- **`exit_partial` 잔량이 같은 시그널 수명 안에서 재매도 불가** — 멱등키가 (일자·심볼·side)라 잔량 주문이 같은 키를 쓴다. 회귀는 아니다(조치 전엔 관리셋을 떠나 영영 못 팔았다). 근본 해결은 개선5 설계 변경
- **같은 날 *정상 종료* 실발주 재실행이 원장을 덮어씀** — 종료 경로별 접미사로 나머지는 갈랐다. `run_id` 필요
- **exit 최소수량(dust) 하한 미도입** — 잔량 분포 미측정. 지금 정하면 추측 상수다
- **`_collect_fills` 페이싱 부재** — `_get` 백오프가 재시도하므로 지연이지 유실이 아니다. 올바른 해법은 어댑터 배치 조회(인터페이스 변경)
- **kill switch가 `_build_plan` 뒤에 확인됨** — 최상단으로 옮기면 dry-run도 막혀 계획 검토가 불가능해진다
- **실현손실 미계상** — `record_loss` 프로덕션 호출 0건. 평단가(`cost`, holdings 전용)를 스냅샷·영속하는 단계가 선행돼야 한다
