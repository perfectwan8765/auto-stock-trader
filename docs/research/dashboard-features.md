...
> ⚠️ **전제가 바뀌었다 (2026-08-16).** 이 조사는 대시보드를 실운영 앱으로 확장하려는 목적으로
> 수행됐는데, 그 뒤 **Edge v2가 1층 게이트에서 종료돼 굴릴 전략이 없다**(`../project/microcap-insider-prereg.md` §10).
> 대시보드 확장은 **보류**다.
>
> 다만 **§2(기존 문서 정정 10건)와 §3(수렴 기능)은 전략과 무관하게 유효**하다. 그중 반영된 것:
> **#5 환전 시간창** → `roadmap.md` Phase 0 실측 상수 · **#6 safe state = frozen** → `roadmap.md` 개선4.
> #2·#3(ledger `intent`·`unknown` 상태)은 원장 구현 시점에 반영할 것.
>
> **§6 UNVERIFIED 중 U10은 해소됐다** — Phase 0 실주문으로 "고정 최소수수료 없음"이 확인됐다
> ($1 주문 commission 0, $66.49에 $0.06 = 0.090%). **U8(settlementDate 기준)은 여전히 미확인**이며
> 첫 실거래 때 확인해야 한다.
...
>   암묵 가정 위에 서 있었다. 사용자가 앱 자체를 1급 목표로 확정함에 따라 해당 가정은 무효.
...
3. 서로 다른 도메인의 조사가 같은 결론에 수렴한 기능이 **8개** 있다. 수렴은 강한 설계 신호다. → §3
...
7. **라이브 성과로는 엣지를 검증할 수 없다.** Lo(2002) 기준 SR=0.5를 t=2로 확인하려면 약 16년이 걸리는데
   주간 리밸런싱 1년은 52관측뿐이다. 모니터링 도구군 전체가 측정 불가능한 것을 측정한다. → §9-4
8. **과잉 엔지니어링 위험 8/10.** 후보 32개 중 수익을 개선하는 항목이 0개다. → §9-5
...
|---|---|---|---|
| 1 | **의사결정 시점 입력 + `arrival_px`** | 슬리피지·implementation shortfall 분해 | 흔히 말하는 "장중 가격을 못 구해서"가 아니다. yfinance adjusted price는 배당·분할 때마다 과거 전체가 소급 수정된다 → "그때 모델이 본 숫자"는 같은 소스로 재현되지 않는다(point-in-time / restatement bias). MARKET 주문이라 지정가 기록조차 없다 |
...
1차 조사가 "지금 안 잡으면 잃는다"로 분류한 8항목이 실제로는 **전부 사후 회수 가능**했다.
대부분 `GET /orders/{id}.execution`으로 되찾을 수 있고, 파생 지표는 원시 이벤트에서 언제든
다시 계산된다. 저장은 **편의이지 보존이 아니다.**

예외 하나 — `fx_conversions`(수동 환전 기록)는 잔고 델타로 역산은 되지만 API에 환전
엔드포인트가 없으므로 **기록 습관은 지금 들이는 편이 낫다.**

<details>
<summary>반박 8건 전문 (1차 주장 ↔ 실제) — 심각도 재조정의 근거이므로 삭제하지 않고 접어 둔다</summary>
...
마지막 항목이 구조적으로 중요하다. 정보 손실은 **이벤트 → 집계** 방향으로만 발생하고 반대는 언제든 된다.
...
| 4 | qlib-toss — "T+2 실측 확정" | 측정은 맞다. 단 미국 현지는 2024-05-28부터 T+1이고 T+2는 국내 예탁·외화결제 버퍼다. **상수로 박지 말고 API `settlementDate`를 그대로 저장할 것.** 미국 휴장일 + 한국 휴장일이 겹치면 규칙 계산이 어긋난다 | 🔴 |
| 5 | qlib-toss — "자동환전 X, 선환전 필요" | 맞다. 추가로 **환전 시간창이 비용을 10배 가른다**: 평일 09:00~15:30 KST(서울외환시장 영업일) = 스프레드 0.05%, 그 외/주말/공휴일 = 0.5%. 문서에 없는 운영 제약 | 🔴 |
| 6 | 개선4 kill switch | **safe state가 미정의.** long-only 현물의 안전 상태는 `frozen`(동결)이다. `flat`(청산)은 안전 상태가 아니다. 자동 청산은 왕복 20bp를 확정 실현시키고 저점매도를 고착시킨다. **자동 손실 청산 로직은 넣지 말 것** | 🟠 |
...
| **헬스체크 — 조용한 미실행 탐지** | OSS·기관 | 주 1회 잡의 최대 실패 모드는 **아예 안 돈 것**이다. 크래시가 아니다 |
...
5. **Lookahead-analysis.** 정적 코드 분석이 아니라 *전체 백테스트 vs 개별 시그널 백테스트*의 결과 비교로
   미래참조를 잡아내고 편향된 지표명까지 지목. (문서가 스스로 위음성 가능성을 경고한다. 통과했다고 안전 결론 금지)
...
기관 통제의 대부분은 **자본 규모**에 비례한다. 아래는 다르다. 실패 모드에 비례하고
...
이유는 전송은 성공했는데 응답을 못 받는 상태가 반드시 존재하기 때문.
...
$700에서도 필요한 이유는 API가 체결을 놓칠 수 있기 때문이다: 부분체결, 응답 타임아웃,
소수점 반올림, 그리고 무엇보다 **corporate action(분할·병합·배당)**.
어긋난 상태로 다음 주 리밸런싱이 돌면 오차가 주문 수량에 증폭되어 들어가므로
원장 기반 봇은 대사 없이는 오차가 누적되는 시스템이다.

**3. Append-only audit trail + reason code**
1인 봇에서 감사의 대상은 규제기관이 아니다. **6개월 뒤의 본인**이다.
"이 종목을 왜 안 샀지?", "왜 이 주에 벤치마크와 갈렸지?"에 답하려면
이벤트를 덧붙이는 테이블이 필요하다. 상태를 덮어쓰는 테이블로는 안 된다.
...
> - long-only 현물은 마진콜도 강제청산도 무한손실도 없어 아무것도 안 하는 것이 안전한 상태다
>
...
RTS 6 Art.15의 4개 통제 중 **maximum order value + 배치 총 notional 상한**만 있으면
...
이 화면의 목적은 회전율 억제 근거 제공이다. 실행 품질 개선이 아니다.
...
4. **T+2 vs T+1 불일치는 실재하며 정상이다.** 미국 현지 T+1(2024-05-28~), 국내기준 T+2(기존 T+3→T+2).
...
⚠️ 토스 API `/api/v1/exchange-rate`의 `midRate`는 1분 갱신 실시간 참고값으로,
...
> 세금 계산 기능의 본질은 "지금 계산"이 아니라 "나중에 계산 가능하도록 데이터를 축적"하는 것이다.
> 계산 로직보다 **필드 캡처를 먼저** 구현할 것.
>
> 토스증권은 무료 신고대행을 제공하고 10개 증권사 합산 신고가 가능하다.
...
**다중검정 허들**: Harvey·Liu·Zhu에 따르면 신규 팩터는 t>2.0이 아니라 **t>3.0**.
...
> "IC/분위수익/회전율/자기상관"은 대시보드에 노출만 하면 된다. 새로 만들 것이 아니다.
...
목표는 "이 전략은 회복장에서만 작동한다"를 한눈에 보는 것이다. 커밋 `3278dde`에서 손수 도달한 결론의 자동화.
...
주어진 오경보율에서 변화 탐지가 최속임이 증명됨. 현재 3개 대륙 $500B+ 운용자산 모니터링에 사용.
...
> 전제는 주간 초과수익의 근사 무상관이다. SPY를 제대로 잡으면 성립.
>
...
백테스트가 포착하는 것은 주로 공통 팩터 레짐, 그것도 **런칭 직전에 존재하던** 레짐일 뿐이며
전략 고유 스킬이 아니다. 극단적 팩터 랠리 직후 런칭일수록 백테스트를 더 크게 할인해야 한다.
...
[scripts/live/rebalance.py](../../scripts/live/rebalance.py)는 모델 파일 경로가 아니라 레지스트리 버전으로 로드하고
...
핵심은 스택 선택 자체가 아니다. **선택을 되돌릴 수 있는 상태로 만드는 것**이 core 분리의 목적이다.
...
1차 조사가 뽑은 항목은 전부 *주문을 낸 뒤* 무엇을 기록·대조할지에 집중돼 있다.
...
두 번째 계정(~$10k 손실)도 동일 패턴이다. 감정이 거래를 중단시켜 설계 대비 미달.
...
**FINRA Regulatory Notice 15-09**가 배포 전 요건을 가장 구체적으로 열거한다:
...
**FIA (2024-07)** 는 통제를 이름으로 못 박는다: §1.1 Maximum Order Size
...
**Robert Carver, pysystemtrade `docs/production.md`** 는 이 목록 중 유일하게 개인 트레이더가 실제 운영하는 1차 문서다:
...
**Kevin Davey** 는 라이브 전환을 수치로 말하는 거의 유일한 실무자다:
...
> 실측값을 보면 반대다. 최소주문 ≤$1, 소수점 가능, $1 체결 시 commission $0이어서
...
현실적 대체물은 회귀 테스트다. 실제 브로커 응답을 녹화해 고정 픽스처로 삼으면 된다.
...
그 전까지 이들이 내는 건 노이즈다. 노이즈에 반응해 전략을 손대는 것이
...
| 이동 | 항목 | 이유 |
|---|---|---|
| 모니터링군 → **라이브 전** | **비용·회전율 예산 1회 계산** | 실측 0.10%/편도 + FX 3bp + 주간 리밸. **연 3%대 비용 드래그 추정**(회전율 가정에 의존하므로 백테스트 회전율로 확정 필요). $700에서 이건 대시보드 주제가 아니라 **전략을 돌릴지 말지의 go/no-go 입력** |
...
나머지 중 무엇이 실제로 필요한지를 관측으로 알게 된다.
...
Phase A와 **병행**해도 무방하다. 미루라는 게 아니라 라이브를 막지 않는다는 뜻이다.
