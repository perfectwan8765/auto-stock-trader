...
>
> ⚠️ 그리고 이 표의 **모든 숫자는 분할 embargo 도입 이전** config에서 나온 값이다(커밋 `977d22a`).
> 누수 규모가 작아 결론은 바뀌지 않지만 **config ↔ 결과 연결은 끊긴 상태**이며, 재실행은
> [`docs/project/trial-ledger.md`](docs/project/trial-ledger.md)에 시행으로 계상한 뒤 한다.

| 실험 | 핸들러 + 모델 | **R²_oos** | **게이트** | IC | Rank IC | 초과(비용후,연) | IR | MDD |
|------|--------------|---:|---|-----|---------|------|-----|-----|
| 베이스라인 | Alpha158 + LightGBM | **+0.00002** | ❌ C | 0.0121 | 0.0084 | -5.6%² | -0.35 | -29.0% |
