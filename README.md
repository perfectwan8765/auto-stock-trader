...
| 실험(단일시드) | Alpha360 + GRU | 0.0090 | 0.0096 | +10.8%¹ | 0.71¹ | -20.5% |
| 참고(중국)² | qlib CSI300 최고 (HIST/Alpha360) | 0.052 | 0.067 | — | 1.37 | -6.8% |
| 기준 | SPY | — | — | 0 | — | — |

¹ **단일 시드라 신뢰 불가.** Rank IC ≈ 0.01(예측력 사실상 0)인데 초과 +10.8%는 서로 정합하지 않는다 — 신호가 아니라 단일 백테스트의 변동성/우연이다. 베이스라인(-5.6%)과 GRU(+10.8%)가 Rank IC는 똑같이 ~0인데 성과만 16%p 벌어진 것이 그 증거. 신뢰하려면 qlib처럼 20시드 평균±표준편차가 필요하고, 그러면 ~0으로 회귀할 가능성이 크다.
² 중국 A주 CSI300 백테스트(20시드)라 미국과 직접 비교 대상이 아니다 — 도달 가능 상한의 감(感)만.

**결론**: 두 모델 모두 미국 대형주 주간에서 **Rank IC ≈ 0.01 → exploitable 엣지 미검출**(중국 벤치 0.067의 1/7). 병목은 모델이 아니라 시장·신호다.
